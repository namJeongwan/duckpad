...
| Focused and serial validation | Passed | TabFlow 93/93, insertion 5/5, editor-group commands 29/29, Scintilla groups 24/24, and Language editor 59/59 pass. The serial Debug and Release runs each complete all 668 tests in 12 suites; deprecation-as-error Scintilla builds pass in both configurations |
| Current-source packaged smoke | Passed | A fresh hidden Release app passed the production language smoke, including exact UTF-8/CRLF multi-character insertion, and the 50-tab multiline smoke (`6` rows). No smoke process remained |
| Default parallel whole suite | Non-release baseline limitation | Process-global AppKit tests are not treated as parallel-safe. The attributable whole-suite gate is the successful 668-test serial run in both Debug and Release; two stale test-window animation lifetimes found during the earlier gate remain fixed and covered |
| Reviewed source delivery | Code/test source complete | Commits `0dc85b7` through `ca97721` each have an exact signed review receipt and audit. Cumulative pre-push review reports 0 Critical / 0 Important / 0 Minor. Before documentation commits, the code/test checkpoint was verified on both local history and remote `feature/editor-groups-compare` as `ca97721d8795a647c8677ab339cdfd5b49ff4182` |
...
| Direct-input delimiter pairing | Complete | `()`, `[]`, `{}`, `''`, `""`, and backticks close atomically; one undo and one recovery revision |
| Exact closer skip-over | Complete | `)`, `]`, `}`, quote, double quote, and backtick advance the caret without bytes, revision, or Undo mutation |
| JSON and Python smart newline | Complete | Current-line indentation, configured tabs/spaces, and aligned JSON closer |
| Real AppKit input path | Complete | Queued key events prove brace, quote, backtick, and matching-closer behavior through the Scintilla first responder |
| Focused validation | Complete | Debug/Release language suite, split-pane and multi-character CRLF regressions, builds, and production language smoke pass |
| Repository-wide validation | Complete | Serial Debug and Release runs each pass all 668 tests in 12 suites |
| Vendored Cocoa compatibility | Complete | Debug and Release Scintilla bridge builds pass with deprecated declarations promoted to errors |
