...
| Native command and tab chrome | Popup-anchor correction validated | Compact command buttons open the exact native `NSMenu` with its content top explicitly anchored immediately below the window-local bar. Tabs keep full intrinsic title widths and compact spacing, leave ordinary trailing room instead of stretching to 100%, and add a full-height row only when the next complete tab no longer fits. Tab scrolling and scroller chrome remain disabled |
...
| Focused and serial validation | Passed | Compare 21/21, editor-group commands 27/27, TabFlow/AppKit 85/85, layout model 15/15, workspace 16/16, Scintilla group 23/23; full serial suite exits 0 across 640 discovered tests; Debug/Release builds pass |
