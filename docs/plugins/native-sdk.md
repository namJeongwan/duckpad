...
One instance is owned per authorized service command across Duckpad windows. Opening the panel in another window moves the same view. Closing the panel keeps clipboard collection active. Disabling/revoking the plugin or closing its last owning window stops and destroys its instances. Removing or changing the selected package so it no longer verifies also stops activation. Pending insert tokens are invalidated when the panel closes or moves. The host routes File → Close and Command-W to the dock when keyboard focus is inside its plugin view, including a search field’s shared field editor. This closes only the panel and restores editor focus; collection stays active. With focus in the editor, normal document-close behavior remains in effect. Plugins may also provide their own close button through `close_panel`.
