...
The application delegate retains every live `DuckpadWindowController` by stable object identity. Controllers detach synchronously when their native windows close. A normal red-window close flushes and hides the registered workspace without dirty-document prompts. Dock reopen reuses that workspace, and hidden workspaces remain within the live-window cap. `Command-Q` synchronously admits every live window that requires review, locks their mutation surfaces, preserves dirty documents without save/discard prompts, and performs each final recovery flush before answering AppKit. If startup or a recovery flush fails, every admitted window is reopened for editing and the application remains alive. Red-close and application-quit requests that overlap join the same review task.
