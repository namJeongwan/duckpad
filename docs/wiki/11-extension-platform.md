...
Status: **Phase 8 approved, committed and audited** (`1f60bedd`); Phase 26 XPC packaging transition is implemented and independently review-pending.

## Supported boundary

Duckpad loads declarative `.duckpad-plugin` directories and runs a WebAssembly module in a separate runtime. SwiftPM development/tests use `DuckpadPluginHost`; packaged applications use the embedded `DuckpadPluginRuntime.xpc` service. The MVP supports commands, keybinding metadata, snippets, themes, and language metadata. Command execution is the only active contribution today; declarative metadata is parsed and bounded but runtime merge remains deferred until each subsystem has a conflict policy.
...
The SwiftPM `Process` helper transport remains deliberately labeled **Developer Preview** and is absent from the `.app`. Production composition uses an application-owned XPC service signed inside the outer bundle. The service receives only one bounded value frame, has App Sandbox as its sole entitlement, and has no URL/file handle/environment/native plugin object in its protocol. User-extension execution remains disabled by default; the exact built-in signed sample exercises the production XPC route.

Deferred: WASI, JIT/AOT, native libraries, filesystem/network/process/environment/clock imports, zip/network installer, background extension activation, arbitrary webviews, debugger API, and full VS Code/Notepad++ plugin API compatibility. Credentialed Developer ID notarization is a release operation, not a source-code capability.
...
- Reproduction: `scripts/vendor_wamr_2_4_5.sh` requires an absent, non-symlink target, validates TLS/checksum/archive paths and vendored-subset links, applies the exact documented `MAP_JIT` and atomic instruction-cancellation patches, stages privately, and publishes with Darwin `RENAME_EXCL`. `--verify` regenerated and byte-compared all 168 checked-in files, including `PROVENANCE.md`.
...
- IPC is protocol-versioned JSON with a four-byte big-endian length, frame caps, single-flight request IDs, and per-request timeout/cancel. Production sends the same value frame through `NSXPCConnection`; interruption, invalidation, reply, timeout, and cancellation share a first-wins continuation. The listener remains responsive on a serial worker queue, and timeout/cancel atomically trips WAMR's classic-interpreter instruction poll so a tight loop terminates before immediate reuse; clean process retirement is the bounded fallback. Logical cancellation and the original wall-clock deadline survive restart attempts. Development Process transport additionally enforces exact EOF/stderr caps and bounded TERM/KILL/reap through one shared exit signal.
...
Acceptance covers signed-bundle/runtime resolution; user-root trust downgrade; hidden/tampered/duplicate/nested/oversized packages; zero-import/memory/table/start/ABI/LEB policy; null bridge input; strict framing; private generation policy; identity-bound consent, disable/revoke/reset, least-privilege payload and output containment, busy/cancel and stale-tab behavior; held-transaction authority withdrawal; process timeout/cancel PID reap; dynamic authorized menu and disclosure; bounded 50/500 MiB capture; and a real Scintilla grouped-edit/undo/recovery round trip. Phase 26 adds a shared runtime executor equivalence test and a real signed-loader → sandboxed XPC → non-JIT WAMR → scoped grouped-edit packaged smoke. Its adversarial packaged path proves tight-loop timeout, explicit cancellation, and immediate valid-command recovery. Debug and Release modules now pass 349/349.

## Phase 26 production transition

- `DuckpadPluginRuntimeCore` is the single WAMR executor used by both transports; the development helper no longer owns a second execution implementation.
- Apple Silicon WAMR mapping requests `MAP_JIT` only when JIT/Fast-JIT/AOT is compiled in. Duckpad compiles all three out, so the XPC service runs without any executable-memory entitlement.
- The XPC listener serializes WAMR on a worker queue while remaining cancellation-responsive. Atomic instruction polling ends a looping guest, and the app transport enforces single flight, bounded frames, original-deadline timeout/cancel, restart-safe logical cancellation, and connection invalidation.
- Bundle verification proves the developer helper is absent, the service entitlement allowlist contains only App Sandbox, both Mach-O files have Hardened Runtime, and the nested/outer signatures are valid.
