...
struct LanguageEditorAdapterTests {
    ...
    @Test @MainActor func editorFontPreferencesReachClonesAndPreserveUndoAndZoom() throws {
        ...
        let settings = AppSettings(editorFontName: "Monaco", editorFontSize: 19)
        adapter.applyPreferences(settings)
        #expect(primary.editorFontName == NSFont(name: "Monaco", size: 19)?.fontName)
        #expect(primary.editorFontSize == 19)
        ...
        adapter.activateEditorGroup(group)
        let clone = try #require(adapter.activeScintillaView)
        #expect(clone.editorFontSize == 19 && clone.editorFontName == primary.editorFontName)
        #expect(adapter.applyLanguage(.init(languageID: .plainText, lexerName: "null", indentation: .init(), folding: false, braceMatching: false)))
        #expect(clone.editorFontSize == 19)
        #expect(try sendTestingScintillaMessage(2485, wParam: 32, to: clone) == 19)
        #expect(try sendTestingScintillaMessage(2485, wParam: 0, to: clone) == 19)
        clone.undo()
        #expect(primary.contentUTF8 == Data("base".utf8))
        ...
        #expect(adapter.activeScintillaView?.editorFontSize == 19)
